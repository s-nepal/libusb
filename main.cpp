...
int main() {
	...
	cout << "Number of devices: " << cnt << endl;
	for (int i = 0; i < cnt; i++) {
	    cout << "Device Address: " << (double)libusb_get_device_address(devs[i]) << " ";
	    cout << "Port Number: " << (double)libusb_get_port_number(devs[i]) << " ";
	    cout << "Device Speed: " << libusb_get_device_speed(devs[i]) << endl;
	}
